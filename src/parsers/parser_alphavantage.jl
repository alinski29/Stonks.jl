...
function parse_alphavantage_price(content::AbstractString; kwargs...)::Union{Vector{AssetPrice},Exception}
  ...
  if !haskey(js, key_data)
    return KeyError("Did not find expected key: '$key_data'")
  end
  series = collect(js[key_data])
  ticker = js[key_meta]["2. Symbol"]
  if (!haskey(first(values(js[key_data])), "4. close"))
    return KeyError("Did not find expected key: '4. close'")
  end
  args = Dict(kwargs)
  ...
  if isempty(series)
    return DomainError(from, "There are no datapoints between '$from' and '$to'")
  end
  return [AssetPrice(
    symbol = ticker,
...
function parse_alphavantage_info(content::AbstractString; kwargs...)::Union{Vector{AssetInfo},Exception}
  ...
  typeof(maybe_js) <: Exception && return maybe_js
  js = maybe_js
  !haskey(js, "Symbol") && return KeyError("Symbol")
  !haskey(js, "Currency") && return KeyError("Currency")
  [AssetInfo(
    symbol = js["Symbol"],
