...
struct WriteTransaction
  ...
  function WriteTransaction(ops, format, dest) 
    tmp_dir = "$(joinpath(@__DIR__, dest))/.tmp"
    bkp_dir = "$(joinpath(@__DIR__, dest))/.bkp"
    new(ops, format, dest, tmp_dir, bkp_dir)    
  end
end
...
function execute(tr::WriteTransaction, writer::Function)
  ...
  if !success
    cleanup(tr)
    return (false, ErrorException("Failed to execute transaction. Write to temp directory failed"))
  end
  try
    commit(tr)
  ...
  try
    ...
    bkps = @chain begin 
      ...
      map(op -> (
        ...
        src=!isempty(op.key) ? "$(tr.dest)/$(op.key)/$(op.filename)" : "$(tr.dest)/$(op.filename)", 
        dest=!isempty(op.key) ? "$(tr.bkp)/$(op.key)/$(op.filename)" : "$(tr.bkp)/$(op.filename)"
      ),
    _)
    end
    @sync begin
